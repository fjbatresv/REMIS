...
class ExcelController extends ListController
{
    ...
    public function excelAction()
    {
      ...
      $response->headers->set('Content-Disposition', 'attachment;filename=admin_export_lista_de_colores.xlsx');
    ...
    protected function createExcelheader(\PhpExcel_Worksheet $sheet){
        ...
        $sheet->getStyle($columnLetter . '1')->getFont()->setBold(true);
        $sheet->getColumnDimension($columnLetter)->setAutoSize(true);
                      $sheet->setCellValueByColumnAndRow(2, 1, "Created by", true);
        $columnLetter = \PHPExcel_Cell::stringFromColumnIndex(2);
        $sheet->getStyle($columnLetter . '1')->getFont()->setBold(true);
        $sheet->getColumnDimension($columnLetter)->setAutoSize(true);
                      $sheet->setCellValueByColumnAndRow(3, 1, "Updated by", true);
        $columnLetter = \PHPExcel_Cell::stringFromColumnIndex(3);
        $sheet->getStyle($columnLetter . '1')->getFont()->setBold(true);
        $sheet->getColumnDimension($columnLetter)->setAutoSize(true);
                      $sheet->setCellValueByColumnAndRow(4, 1, "Created at", true);
        $columnLetter = \PHPExcel_Cell::stringFromColumnIndex(4);
        $sheet->getStyle($columnLetter . '1')->getFont()->setBold(true);
        $sheet->getColumnDimension($columnLetter)->setAutoSize(true);
                      $sheet->setCellValueByColumnAndRow(5, 1, "Updated at", true);
